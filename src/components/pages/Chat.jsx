...
const Chat = () => {
 ...
 let [timer, setTimer] = useState(0)
  ...
  if(user?.sub != chats.userId1 && user?.sub != chats.userId2) return navigate("/")
  return (
    <>
    ...
    <div className="chat-header flex justify-between py-3 px-2 bg-slate-50 items-center shadow-md">
      ...
      <div onClick={()=> navigate("../user/" + chats.postedBy._id)} className="posted-user-info flex items-center cursor-pointer">
      <div className="profile-image mx-2 mt-1 cursor-pointer">
        <img className="w-10 rounded-full" src={reqUser.image} alt="user-image" />
      </div>
      <div className="name ml-1 font-bold cursor-pointer">{reqUser.userName}</div>
      </div>
      </div>
      <div className="right flex items-center space-x-2">
     {reqUser?.mobileNumber && <div className="call cursor-pointer">
        <a href={"tel:+91"+ reqUser?.mobileNumber}><CallIcon /></a>
          ...
          <>
            <div ref={ref} className="py-8 w-full px-1 singleChat">
              <div className="chat-message">
               ...
               <Avatar alt={reqUser.userName} src={reqUser.image} sx={{ width: 25, height: 25 }} />
