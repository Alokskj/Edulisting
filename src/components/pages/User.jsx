...
const User = () => {
  ...
  useEffect(() => {
    ...
    client.fetch(query).then((data) => {
      setUser(data[0]);
      const followArray = Array.isArray(data[0]?.following)
        ? data[0]?.following?.map((e) => e._ref)
        : [];
    ...
    if (!following) {
      ...
      setUser((prevUser) => ({
        ...
        following: prevUser.following.filter(
          (follower) => follower._ref !== currentUser?.uid
        ),
      }));
      // Perform API call to update the followers in the database
      client
    ...
    <>
      <Transition>
        <Helmet>
          ...
          <meta
            property="og:title"
            content={user?.userName + "'s Profile - Edulisting"}
          />
